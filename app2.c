...
int main(){
  ...
  uint64_t  id=9876539996846;//mon id
  ...
  if ((status = getaddrinfo("vps-63c87489.vps.ovh.net", NULL, &hints, &res)) != 0) {
	...
	//testé la sorie du select         
		...
		if(req[point]==5){
			...
			if(req[point]==8){
				...
				listeDonnees *dc=malloc(sizeof(listeDonnees));
				memcpy(dc->data.i,&req[point+2],8);
				...
				unsigned char hash_recue[16];				
				memcpy(hash_recue,&req[point+12],16);			
                                memcpy(dc->data.s,&req[point+10],2);
                                unsigned char data_recue[192]; 
                                memcpy(dc->data.d,&req[point+26],x-26);	
				unsigned char hash_test[16];
				nodehash(hash_test,&dc->data);
